...
`lda()` Linear Discriminant Analysis (from the MASS package)
`qda()` Quadratic Discriminant Analysis (from the MASS package)
`naiveBayes()` Naive Bayes (from the e1071 package)
...
The resulting error rate is not very good but not a surprising result for trying to predict future stock market conditions only based on the last few days.
Below the analysis is repeated, but only keeping the predictors Lag 1 and Lag 2 which had the smallest P-values (i.e. higher significance)
```{r}
glm.fits <- glm(
